...
import { 
  ...
  FileText, ChevronRight, ArrowLeft, BadgePoundSterling 
} from 'lucide-react';
import { StaffMember, Client, ComplianceRecord } from '../types';
import { MOCK_STAFF, MOCK_CLIENTS } from '../services/mockData';
...
const People: React.FC = () => {
  ...
  const filteredClients = MOCK_CLIENTS.filter(c => c.name.toLowerCase().includes(searchTerm.toLowerCase()));

  // Derived State
  const selectedStaff = activeTab === 'staff' ? MOCK_STAFF.find(s => s.id === selectedId) : null;
  const selectedClient = activeTab === 'clients' ? MOCK_CLIENTS.find(c => c.id === selectedId) : null;
    ...
    <div className="animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
         ...
         <section className="grid grid-cols-2 gap-4">
            <button className="p-3 border border-slate-200 rounded-lg hover:bg-slate-50 text-sm font-medium text-slate-700 flex items-center justify-center gap-2">
           ...
           <div className="relative w-full max-w-md bg-white h-full shadow-2xl p-6 animate-in slide-in-from-right duration-300">
              ...
              <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setIsAddModalOpen(false); }}>
                 ...
                 <div>
                    ...
                    <input type="email" className="w-full p-2 border border-slate-300 rounded-lg" />
                 </div>
                 {activeTab === 'staff' ? (
                   <div>
