 ...
 * Displays staff members and clients with search and detail views.
 */

import React, { useState } from 'react';
import {
  Search, UserPlus, Phone, Mail, MapPin,
  ShieldCheck, AlertTriangle, X, FileText, 
  ChevronRight, ArrowLeft, BadgePoundSterling, Pill
} from 'lucide-react';
import { StaffMember, Client, ComplianceRecord } from '../types';
import { useStaff, useClients, useCreateClient, useCreateStaff } from '../hooks';
import { StatusBadge, CareLevelBadge } from '../components/shared/ui/StatusBadge';
import { StaffListItem } from '../components/people/StaffTable';
import { ClientListItem } from '../components/people/ClientsTable';
...
const People: React.FC = () => {
  ...
  const { data: staffList = [], isLoading: staffLoading, error: staffError } = useStaff();
  ...
  const loading = activeTab === 'staff' ? staffLoading : clientsLoading;
  const error = activeTab === 'staff' ? staffError : clientsError;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (activeTab === 'staff') {
        ...
        log.track('staff_created_from_form');
      } else {
        await createClient.mutateAsync({
          name: formData.get('name') as string,
          address: formData.get('address') as string,
          careLevel: formData.get('careLevel') as Client['careLevel'],
          fundingDetails: {
        ...
        log.track('client_created_from_form');
      }
      
      setIsAddModalOpen(false);
    } catch (err) {
      log.error('Failed to create entity', { error: err, type: activeTab });
    }
  };
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              {activeTab === 'staff' ? (
                <div>
                  ...
                  <select name="role" className="w-full p-2 border border-slate-300 rounded-lg">
                    <option>Carer</option>
              ...
              <button 
                type="submit"
                disabled={createClient.isPending || createStaff.isPending}
                className="w-full py-3 bg-primary-600 text-white font-bold rounded-lg mt-6 hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {(createClient.isPending || createStaff.isPending) ? 'Saving...' : 'Create Profile'}
              </button>
            </form>
          </div>
        </div>
