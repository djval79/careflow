    ...
    <div className="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4">
       {!generatedQuiz ? (
          <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm text-center space-y-6">
             ...
             <div>
                <h2 className="text-2xl font-bold text-slate-900">AI Knowledge Check</h2>
                <p className="text-slate-500 mt-2">Generate a quick 5-question quiz on any care topic to test your knowledge.</p>
             </div>
             
             <div className="relative max-w-md mx-auto">
                <input 
                   type="text"
                   placeholder="e.g. Medication Safety, Fire Procedures..."
                   className="w-full p-4 pr-12 border border-slate-300 rounded-xl shadow-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none"
                   value={quizTopic}
                ...
                <button 
                  ...
                  className="absolute right-2 top-2 bottom-2 px-4 bg-purple-600 text-white rounded-lg font-bold hover:bg-purple-700 disabled:opacity-50"
                >
                   {isGeneratingQuiz ? <Loader2 className="animate-spin" size={20} /> : 'Go'}
                </button>
             </div>
          </div>
       ) : (
                ...
                {!showScore ? (
                   ...
                   <div className="mt-8 p-6 bg-slate-900 text-white rounded-xl text-center animate-in zoom-in">
                      ...
                      <p className="text-slate-400 mb-4">
                         {calculateScore() === 5 ? 'Perfect score! You are a master.' : 'Good effort! Review the answers above.'}
                      </p>
                      <button 
                        onClick={() => setGeneratedQuiz(null)}
                        className="px-6 py-2 bg-white text-slate-900 font-bold rounded-lg hover:bg-slate-100"
                      >
                         Try Another Topic
                      </button>
                   </div>
