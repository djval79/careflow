...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/jpeg", "image/jpg", "image/png"];
    bucketConfig.file_size_limit = 2097152;
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for biometric-photos" ON storage.objects FOR SELECT USING (bucket_id = 'biometric-photos');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for biometric-photos" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'biometric-photos');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for biometric-photos" ON storage.objects FOR UPDATE USING (bucket_id = 'biometric-photos');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for biometric-photos" ON storage.objects FOR DELETE USING (bucket_id = 'biometric-photos');`
    ];
