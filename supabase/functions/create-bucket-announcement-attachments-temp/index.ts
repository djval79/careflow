...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for announcement-attachments" ON storage.objects FOR SELECT USING (bucket_id = 'announcement-attachments');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for announcement-attachments" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'announcement-attachments');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for announcement-attachments" ON storage.objects FOR UPDATE USING (bucket_id = 'announcement-attachments');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for announcement-attachments" ON storage.objects FOR DELETE USING (bucket_id = 'announcement-attachments');`
    ];
