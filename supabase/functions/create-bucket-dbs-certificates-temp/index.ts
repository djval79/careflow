...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'dbs-certificates',
        name: 'dbs-certificates',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["application/pdf", "image/jpeg", "image/jpg", "image/png"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for dbs-certificates" ON storage.objects FOR SELECT USING (bucket_id = 'dbs-certificates');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for dbs-certificates" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'dbs-certificates');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for dbs-certificates" ON storage.objects FOR UPDATE USING (bucket_id = 'dbs-certificates');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for dbs-certificates" ON storage.objects FOR DELETE USING (bucket_id = 'dbs-certificates');`
    ];
