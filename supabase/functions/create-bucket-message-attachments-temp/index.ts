...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Upload for message-attachments" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'message-attachments');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for message-attachments" ON storage.objects FOR UPDATE USING (bucket_id = 'message-attachments');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for message-attachments" ON storage.objects FOR DELETE USING (bucket_id = 'message-attachments');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'message-attachments',
        public: true,
        allowed_mime_types: ["image/*", "application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"],
